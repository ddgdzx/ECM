...
private val copy = mapOf(
    ...
    "all_layers" to listOf("全部层", "全部層", "All layers", "Todas las capas", "Alle Ebenen"),
    "layer_number" to listOf("%d层", "%d層", "Layer %d", "Capa %d", "Ebene %d"),
    "slot_layer_hint" to listOf("选择具体层后，可直接点选被其他层遮挡的格口；带 ✓ 的格口会一起分配给该元件。", "選擇指定層後，可直接點選被其他層遮擋的格口；帶 ✓ 的格口會一起分配給該元件。", "Choose a layer to select slots hidden behind other layers. Slots marked ✓ are assigned together.", "Elige una capa para seleccionar espacios ocultos por otras capas. Los marcados con ✓ se asignan juntos.", "Wähle eine Ebene, um verdeckte Fächer auszuwählen. Mit ✓ markierte Fächer werden gemeinsam zugewiesen."),
