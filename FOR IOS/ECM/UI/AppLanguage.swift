...
enum AppCopy {
    static func text(_ key: String, _ language: AppLanguage) -> String {
        let values: [String: [AppLanguage: String]] = [
            ...
            "all_layers": [.simplifiedChinese: "全部层", .traditionalChinese: "全部層", .english: "All layers", .spanish: "Todas las capas", .german: "Alle Ebenen"],
            "layer_number": [.simplifiedChinese: "%d层", .traditionalChinese: "%d層", .english: "Layer %d", .spanish: "Capa %d", .german: "Ebene %d"],
            "slot_layer_hint": [.simplifiedChinese: "选择具体层后，可直接点选被其他层遮挡的格口；带 ✓ 的格口会一起分配给该元件。", .traditionalChinese: "選擇指定層後，可直接點選被其他層遮擋的格口；帶 ✓ 的格口會一起分配給該元件。", .english: "Choose a layer to select slots hidden behind other layers. Slots marked ✓ are assigned together.", .spanish: "Elige una capa para seleccionar espacios ocultos por otras capas. Los marcados con ✓ se asignan juntos.", .german: "Wähle eine Ebene, um verdeckte Fächer auszuwählen. Mit ✓ markierte Fächer werden gemeinsam zugewiesen."],
